...
9. **tiempo_inicio** y **tiempo_fin**: Sirve para entregar solo las que se encuentran dentro del rango de fechas, es decir, las que tienen como **fecha de inicio** dentro de ese rango. El formato es ```YY-MM-DD-hh:mm:ss``` (Año, mes, dia, hora, minutos, y segundos, en ese orden, cada numero separado por UN caracter, de los cuales puede ser alguno de estos ```:-_```). Ver ejemplo de uso mas abajo.
